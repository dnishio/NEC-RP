...
class AnnoyDictionary(object):
    def __init__(self, dict_size, key_width, new_value_shift_coefficient=0.1, batch_size=100, key_error_threshold=0.01,
        ...
        qu = faiss.IndexFlatL2(int(key_width))
        self.index = faiss.IndexIVFFlat(qu, int(key_width), 100, faiss.METRIC_L2)
        # self.index.set_seed(1)

        self.embeddings = np.zeros((dict_size, key_width), dtype=np.float32)
        self.values = np.zeros(dict_size, dtype=np.float32)
        ...
        self.key_dimension = int(key_width)
    ...
    def _get_k_nearest_neighbors_indices(self, keys, k):
        ...
        #     indices.append(index)
        distances, indices = self.index.search(keys, k=int(k))
    ...
    def _rebuild_index(self):
        ...
        self.index = faiss.IndexIVFFlat(faiss.IndexFlatL2(self.key_dimension), int(self.key_dimension), 100, faiss.METRIC_L2)
        self.index.train(self.embeddings)
        self.index.add(self.embeddings)
    ...
    def _lookup_key_index(self, key):
        distance, index = self._get_k_nearest_neighbors_indices(np.array([key], dtype=np.float32), 1)
...
class QDND(object):
    ...
    def query(self, embeddings, action, k):
        ...
        dnd_embeddings, dnd_values, dnd_indices, dnd_additional_data = self.dicts[action].query(embeddings, k)

        if self.return_additional_data:
            return dnd_embeddings, dnd_values, dnd_indices, dnd_additional_data
        else:
            return np.array(dnd_embeddings, np.float32), np.array(dnd_values, np.float32), dnd_indices
...
def load_dnd(model_dir):
    ...
    with open(os.path.join(model_dir, model_path), 'rb') as f:
        ...
        for a in range(DND.num_actions):
            DND.dicts[a].index = faiss.IndexIVFFlat(faiss.IndexFlatL2(key_dimension), key_dimension, 100, faiss.METRIC_L2)
            ...
            DND.dicts[a].index.train(DND.dicts[a].embeddings)
            DND.dicts[a].index.add(DND.dicts[a].embeddings)
