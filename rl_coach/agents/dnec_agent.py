...
from rl_coach.base_parameters import AlgorithmParameters, NetworkParameters, AgentParameters, MiddlewareScheme
from rl_coach.architectures.tensorflow_components.embedders.embedder import InputEmbedderParameters
from rl_coach.architectures.tensorflow_components.layers import Dense, RPDense
...
class DNECNetworkParameters(NetworkParameters):
    def __init__(self, scheme=MiddlewareScheme.Empty, dense_layer=Dense):
        super().__init__()
        self.input_embedders_parameters = {'observation': InputEmbedderParameters()}
        self.middleware_parameters = FCMiddlewareParameters(scheme = scheme, dense_layer = dense_layer)
        self.heads_parameters = [DuelingDNDQHeadParameters()]
        self.optimizer_type = 'Adam'


class DNECAlgorithmParameters(AlgorithmParameters):
    def __init__(self):
        super().__init__()
        self.dnd_size = 1000000
...
class DNECMemoryParameters(EpisodicExperienceReplayParameters):
    def __init__(self):
        super().__init__()
        self.max_size = (MemoryGranularity.Transitions, 100000)


class DNECAgentParameters(AgentParameters):
    ...
    @property
    def path(self):
        return 'rl_coach.agents.dnec_agent:DNECAgent'


# Neural Episodic Control - https://arxiv.org/pdf/1703.01988.pdf
class DNECAgent(ValueOptimizationAgent):
    ...
    def learn_from_batch(self, batch):
        ...
        if self.ap.algorithm.propagate_updates_to_DND:
            ...
            head.DND.update_keys_and_values(np.zeros(len(embedding_gradients), dtype=np.int32), embedding_gradients, value_gradients, indices)
    ...
    def handle_episode_ended(self):
        ...
        if episode is not None and self.phase != RunPhase.TEST:
            ...
            self.networks['main'].online_network.output_heads[0].DND.add(self.current_episode_state_embeddings,
                                    np.zeros(len(self.current_episode_state_embeddings), dtype=np.int32), returns)
