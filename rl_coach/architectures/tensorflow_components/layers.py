...
class Dense(object):
    ...
    def __call__(self, input_layer, name: str=None, kernel_initializer=None, activation=None, is_training=None):
        ...
        return tf.layers.dense(input_layer, self.units, name=name, kernel_initializer=kernel_initializer,
                               activation=activation, trainable=is_training)

    def __str__(self):
        return "Dense (num outputs = {})".format(self.units)

class RPDense(object):
    """RandomProjectionLayer"""

    def __init__(self, units: int, init_type='gaussian'):
        self.units = units
        init_type_list = ['sparse', 'gaussian']
        if init_type == 'sparse':
            np.random.seed(seed=240)
            value = np.random.choice([np.sqrt(3), 0, -np.sqrt(3)], size=(3136, units), p=[1/6., 2/3., 1/6.])
            self.initializer = tf.constant_initializer(value=value)
        elif init_type == 'gaussian':
            self.initializer = tf.initializers.truncated_normal(stddev=1. / units, seed=240)
        else:
            assert init_type not in init_type_list
            
            raise ValueError('init_type is only allowed : {}'.format(init_type_list))
    ...
    def __call__(self, input_layer, name: str=None, kernel_initializer=None, activation=None, is_training=None):
        ...
        return tf.layers.dense(input_layer, self.units, name=name, kernel_initializer=self.initializer,
                               activation=None, trainable=is_training, bias_initializer=tf.zeros_initializer())

    def __str__(self):
        return "RPDense (num outputs = {})".format(self.units)
